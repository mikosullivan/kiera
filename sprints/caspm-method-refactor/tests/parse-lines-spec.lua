--[[
{
	...
	"role":  "Sprint copy of parse-lines-spec. Exercises the opt-in line-annotation mode of the Caspian → CaspJ transpiler (transpile(source, {lines = true})). Confirms that (a) atom-objects carry a `line` field, (b) statement rows get a trailing `{line: N}` meta-atom, and (c) line numbers reflect the actual source-line each construct appeared on. Points at the sprint's own caspian-caspj so parser changes made under the sprint are exercised here.",
	"run":   "busted sprints/caspm-method-refactor/tests/parse-lines-spec.lua (from repo root)"
}
]]
...
package.path = "./sprints/caspm-method-refactor/src/?.lua;" .. package.path
