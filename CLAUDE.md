...
- Source / test / anything else: `production/src/engine/engine.lua` → `https://puck.uno/production/src/engine/engine.lua` (full repo path, extension kept). Same for `sprints/…`, `tests/…`, `scripts/…`, `web/…`, etc.
...
**Self-check before sending a chat response** that mentions any file: scan for `.md`, or `[name](relative/path)`, or a bare `production/…` / `sprints/…` / `requirements/…` / `documentation/…` / `ideas/…` / `skills/…` / `tests/…` / `scripts/…` prose path — if found, convert. End-of-task summaries that list recently-touched files are the most common slip spot.
...
- Orlando — the docs server that renders this repo at `http://127.0.0.1:8181/` — has moved into its own project at [github.com/mikosullivan/orlando](https://github.com/mikosullivan/orlando) (locally: `/home/miko/projects/orlando/working/`). It reads `~/.orlando/config.json` and serves whatever `content_root` points at; this repo has no `orlando/` subdirectory anymore.
